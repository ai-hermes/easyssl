...
export default function OpenAPIDocs({
  ...
  useEffect(() => {
    fetch(`/api/openapi.json?t=${Date.now()}`)
      .then((r) => {
  ...
  if (loading) {
    return <div className="py-12 text-center text-sm text-[#666]">{t("docs.loading")}</div>;
  }

  if (error || !spec) {
    return (
      <div className="py-12 text-center text-sm text-[#a01616]">
        {t("docs.loadFailed", { error: error || t("common.unknownError") })}
      </div>
    );
  }
                ...
                <div key={id} className="rounded-md border border-[#f1f1f1]">
                  ...
                  {isOpen && (
                    <div className="border-t border-[#f1f1f1] bg-[#fafafa] px-3 py-3 text-sm space-y-4">
                      {op.description && (
                        <div className="text-xs text-[#666]">{op.description}</div>
                      )}
                      {op.consumes && (
                        <div className="flex items-center gap-2">
