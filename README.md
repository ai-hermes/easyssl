...
## Install EasySSL Skill (for Codex/Agents)
...
Create access credentials in Access Management first. In workflow nodes, `accessId` should reference the corresponding access record `id`.
...
### 1) Apply certificate only (no deployment)
...
### 2) Apply (Tencent Cloud) + deploy to Aliyun CAS
...
### 3) Apply (Aliyun DNS) + deploy to Qiniu
